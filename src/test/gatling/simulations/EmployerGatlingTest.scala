...
class EmployerGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Employer entity")
        ...
        .repeat(2) {
            ...
            .put("/api/employers")
            .headers(headers_http_authenticated)
            .body(StringBody("""{"id":null, "name":"SAMPLE_TEXT", "address":"SAMPLE_TEXT", "contactName":"SAMPLE_TEXT", "contactPhone":"SAMPLE_TEXT"}""")).asJSON
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_employer_url")))
            .pause(10)
            .repeat(5) {
